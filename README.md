...
batch 涨 32 倍而单步只涨 2.3 倍——权重读取被摊薄了 32 次，这就是凑批的收益。

这里踩过一个只有看 timeline 才发现的坑：`lm_head_argmax` 原本按 `grid=(B, …)` 起，
每条序列都要把 485 MiB 的 embedding 完整读一遍，B=32 时它一个就占掉整步的 48%。
按 `BLOCK_B` 分块让权重 tile 在 batch 内复用之后，它从 3977us 降到 423us，
几乎不再随 B 增长。
