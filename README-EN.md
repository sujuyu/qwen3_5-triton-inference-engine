...
This is a learning project. The goal is to understand what actually happens at every
step between weights and tokens in a real, non-standard LLM — and what it concretely
takes to make that fast on a GPU.
...
Most kernels were written by hand by the repository author. The exception is the
chunk-64 three-stage parallel prefill for Gated DeltaNet (`gdn_chunk_*`): it was
originally generated by Codex, and later optimized by Claude (replacing the
tensor-core-bypassing `input_precision="ieee"` on a per-operand-dtype basis, and
fixing the blocking in `chunk_output` that had 8 CTAs each recomputing the same
attention matrix — 56x in total), leaving the algorithm itself unchanged. The
author's hand-written sequential recurrence is the baseline for numerical comparison.
