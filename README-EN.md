...
Most kernels were written by hand by the repository author. The chunk-64 three-stage
parallel prefill for Gated DeltaNet (`gdn_chunk_*`) was originally generated by an AI
assistant; its later performance work (tensor-core usage and a blocking fix, 56x) was
done by Claude, leaving the algorithm itself unchanged. The author's hand-written
sequential recurrence is the baseline for numerical comparison.
...
The license for the code in this repository has not been decided yet (there is no
LICENSE file).
