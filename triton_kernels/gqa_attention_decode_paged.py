...
def _paged_kv_append_triton(
    ...
    """
    # TODO(用户填)：grid 从 (H,) 变成 (B, H), 改动只有「取 batch 下标 + 偏基址」. 
    #
    #     pid_b, pid_h = tl.program_id(0), tl.program_id(1)
    #     p    = tl.load(pos_ptr + pid_b)                        # 各自的长度
    #     phys = tl.load(block_table_ptr + pid_b * stride_bt_b + lp)   # 各自那一行页表
    #     k_new_ptr += pid_b * stride_kn_b                       # 各自的新 K
    #     v_new_ptr += pid_b * stride_vn_b
    #
    # **k_cache / v_cache 不加 batch 偏移**——页池是所有序列共用的, 
    # 区分靠的是 block_table 的行, 这正是分页的意义所在. 
    pid_b, pid_h = tl.program_id(0), tl.program_id(1)
...
def _paged_gqa_attention_decode_split_triton(
    ...
    # TODO(用户填)：grid 从 (H_kv, MAX_SPLITS) 变成 (B, H_kv, MAX_SPLITS). 
    #
    ...
    #     q_ptr            += pid_b * stride_q_b
    #     m_partial_ptr    += pid_b * stride_mp_b
    #     l_partial_ptr    += pid_b * stride_lp_b
    #     acc_partial_ptr  += pid_b * stride_ap_b
    #
    # 偏完基址之后, **下面从 num_pages_used 到 tl.store 的每一行都不用改**. 
    # k_cache / v_cache 同样不偏——页池共用. 
    #
    # 注意各条序列的 seq_len 不同, 所以 pages_per_split 也不同：短序列的大部分
    # split 会是零次迭代直接落盘. 实测空转 CTA 约 0.7ns 一个, 可以忽略；
    # 真正贵的是长短不齐造成的尾部拖尾(同样总工作量下倾斜能到 17x). 
...
def _paged_gqa_attention_decode_combine_triton(
    ...
    """
    # TODO(用户填)：grid 从 (H_q, D/BLOCK_D) 变成 (B, H_q, D/BLOCK_D). 
    #
    ...
    #
    # num_active 的三行公式必须和 split kernel 里的切分完全一致, 否则两边对
    # 「哪些 split 有效」的理解不同, 会读到上一次残留的局部量. 
    pid_b, pid_h, pid_d = tl.program_id(0), tl.program_id(1), tl.program_id(2)
    seq_len = tl.load(pos_ptr + pid_b).to(tl.int32) + 1
    num_pages_used = (seq_len + PAGE - 1) // PAGE
    pages_per_split = (num_pages_used + MAX_SPLITS_C - 1) // MAX_SPLITS_C
